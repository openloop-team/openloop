...
the analysis tool sees only an ``input_ref``.  Since Phase 4 that ref is a
**capability token**: the staging CLI generates it with high entropy and
:meth:`InputStore.get` looks it up job-agnostically — possession of the ref is
the authorization, the same posture as ``artifact_ref``.  The orchestrator
provisions only after the approval resolves and its monthly budget gate passes.
"""
...
class InputFile:
    ...
    Filenames are deliberately restricted to one component. The generated
    program receives the files only through the read-only
...
@dataclass(slots=True, frozen=True)
class InputManifest:
    """One operator-staged input set, addressed by its capability token.

    ``input_ref`` alone authorizes the lookup — it is generated with high
    entropy by the staging CLI and never bound to a job (job_id is purely run
    identity; a trusted job-binding channel would double the soft hole where a
    caller-passable id selects another job's data).
    """
...
def materialize_inputs(files: tuple[InputFile, ...], destination: Path) -> None:
    """Write trusted provisioned files into a newly-created inputs directory."""
...
class InMemoryInputStore:
    ...
    def __init__(self) -> None:
        self._by_ref: dict[str, InputManifest] = {}

    async def stage(self, manifest: InputManifest) -> None:
        self._by_ref[manifest.input_ref] = manifest

    async def get(self, input_ref: str) -> InputManifest | None:
        return self._by_ref.get(input_ref)
